...
For each changed src/ file, look for:
  ...
  P1 — Act now (create a follow-up task):
    - Dead code: unused exports, unreachable branches, variables set but never read
    - Duplicated logic: same block (>20 lines) appearing 2+ times — should be extracted
    - Functions >100 lines doing two distinct things — should be split

  P2 — Worth noting but not urgent:
    - Functions 60-100 lines that could be cleaner with early returns
    - Minor inconsistencies in the changed files only

  SKIP:
    - Anything outside the changed files
    - Style preferences (naming, formatting)
    - Adding types or comments
    - Performance micro-optimizations

══ STEP 3: Create follow-up tasks for P1 issues only ═══════════════════════════
If you find P1 issues, append a new task block to the END of {{TASKS_FILE}}.
